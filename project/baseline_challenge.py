...
class BaselineChallenge(FlowSpec):
    ...
    data = IncludeFile("data", default="cloth.csv")
    ...
    def baseline(self):
        ...
        majority_class = 1
        majority_prob = 0.7702075971731449
        class model0:
            def __init__(self): pass
            def predict(self, x): return [majority_class]*len(x)
            def predict_proba(self, x): return [majority_prob]*len(x)
        m0 = model0()
        model0_pred = m0.predict(self.valdf.review)
        model0_prob = m0.predict_proba(self.valdf.review)
        # TODO: return the accuracy_score of these predictions
        acc = accuracy_score(self.valdf.label, model0_pred)

        # TODO: return the roc_auc_score of these predictions
        rocauc = roc_auc_score(self.valdf.label, model0_prob)
